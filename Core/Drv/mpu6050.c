...
#define ALPHA_DPS   0.3f               /* The alpha should be in range [0.0004 : 0.001]*/
...
void MPU6050_Init(MPU6050_TypeDef *mpu, 
    ...
    mpu->acc.offsetRaw[X] = 49.452f;
    mpu->acc.offsetRaw[Y] = 18.354f;
}
...
void MPU6050_Get_Angle(MPU6050_TypeDef *mpu) {
    ...
    Gyro_Get_Dps(mpu->gyro.dps, mpu->gyro.raw, mpu->gyro.offsetRaw);
    Lowpass_Filter_Update(&mpu->gyro.dpsX, &mpu->gyro.dps[X], ALPHA_DPS);
    Lowpass_Filter_Update(&mpu->gyro.dpsY, &mpu->gyro.dps[Y], ALPHA_DPS);
    Lowpass_Filter_Update(&mpu->gyro.dpsZ, &mpu->gyro.dps[Z], ALPHA_DPS);
